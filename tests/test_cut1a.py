...
def test_run_captures_scn_at_start() -> None:
    ...
    def capture(sql: str, *a: object, **kw: object) -> None:
...
def test_insert_uses_as_of_scn() -> None:
    ...
    mock_conn = _make_mock_conn()
    cur = mock_conn.cursor().__enter__()
    # SCN comes from the first fetchone call after 'SELECT current_scn'
                        ...
                        with patch(
                            ...
                            with patch(
                                ...
                                with patch(
                                    ...
                                    engine.run(dry_run=False)  # type: ignore[union-attr]
    _ = RefreshEngine  # silence unused-import linter
...
def test_engine_no_call_timeout_by_default() -> None:
    ...
    engine = _make_engine()
    mock_conn = _make_mock_conn()
    # Track whether call_timeout was assigned. MagicMock will record it; we
    # check the attribute wasn't set to a positive value.
    ...
    # MagicMock returns a MagicMock for unset attrs, so test we never assigned
    # a real number. The clean way: assert no setattr happened on call_timeout.
...
def test_safe_execute_respects_explicit_codes() -> None:
    ...
    # Should not raise.
    engine._safe_execute(mock_conn, "CREATE TABLE X (id NUMBER)", ignore_codes=frozenset({955}))  # type: ignore[attr-defined]
...
def test_introspect_get_identity_columns_querys_all_tab_identity_cols() -> None:
    ...
    # Confirm the right view was queried.
...
def test_engine_resets_identity_sequences_after_insert() -> None:
    ...
    # First fetchone is SCN, subsequent fetchones are MAX(col)+1 etc.
...
def test_engine_sequence_reset_failure_logs_and_continues() -> None:
    ...
    # Run completes; sequence not in reset list.
